#!/bin/bash
#home network scan version 1.0.5, 1.5.2017 arp & nmap
#defining folder V 1.0.5
FOLDER="/home/pi"

#arp scan for IPV4 and MAC adresses V 1.0.5
arp |awk '/ether/{printf $1;}/ether/{print " "$3;}'|sort -V > $FOLDER/arp.list
...
nmap -sP $LAN/$MASK|awk '/Nmap scan report for/{printf $5;}/MAC Address:/{print " "$3;}'|sort -V > $FOLDER/result.list
...
sed -i 's/\b'"$ownIP"'\b/&'" $ownMAC"'/' $FOLDER/result.list
